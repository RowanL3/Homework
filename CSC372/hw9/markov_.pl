...
% This is because there is a hard cap on the number of items in dynamic memory 
...
%  spit back out.
loadUsed([_]).
loadUsed([W|WS]) :- boring(W), loadWord(WS).
loadUsed([W|WS]) :- assert(used(W)), loadUsed(WS).

randomPair(S0,S1):- forall(X, pair(S0, X), L),
			random_member(S1, L).	

% Gives a random word to start the sentence.
randomWord(S):- forall(X, firstWord(X), L),
			random_member(S, L).	
...
next(W, WN) :- swap(W,WS), pair(WS,WN), pair(W,WN), !.
next(W, WN) :- pair(W,WN), !.
next(W, WN) :- randomPair(W,WN), !.

% The core function should return a sentence constructed by markov chains
sentence_(_, 0, ['.']).
sentence_(W1,N,R) :- N1 is N - 1, next(W1,W2), sentence_(W2,N1,R1), append([W1],R1,R).
